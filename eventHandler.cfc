...
<cfcomponent extends="mura.plugin.pluginGenericEventHandler">
	...
	function verifyMuraClassExtension() {
		...
		for(i=1; i<=assignedSites.recordCount; i++ ) {
			...
			// we load all the subtypes for a site
			mySubTypes = application.configBean.getClassExtensionManager().getSubTypes(local.thisSiteID);
			for(j=1; j<=mySubTypes.RecordCount; j++){
				local.thisSubType = application.configBean.getClassExtensionManager().getSubTypeBean();
				local.thisSubType.set( {
					type = mySubTypes.type[j],
					subType = mySubTypes.subtype[j],
